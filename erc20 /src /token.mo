...
shared (msg) actor class Token(
  ...
  private func _transfer(from : Principal, to : Principal, value : Nat) {
    ...
    if (from_balance_new != 0) { balances.put(from, from_balance_new) } else { balances.delete(from) };
  ...
  public shared (msg) func transferFrom(from : Principal, to : Principal, value : Nat) : async TxReceipt {
    ...
    if (allowed_new != 0) {
      ...
      if (allowed != 0) {
        ...
        if (allowance_from.size() == 0) { allowances.delete(from) } else { allowances.put(from, allowance_from) };
      };
    };
    ignore addRecord(
      msg.caller,
  ...
  public shared (msg) func approve(spender : Principal, value : Nat) : async TxReceipt {
    ...
    if (value == 0 and Option.isSome(allowances.get(msg.caller))) {
      ...
      if (allowance_caller.size() == 0) { allowances.delete(msg.caller) } else { allowances.put(msg.caller, allowance_caller) };
