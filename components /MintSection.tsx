...
const MintSection = () => {
  ...
  const { writeAsync: mintA } = useContractWrite({
    address: `0x${ATOKEN_ADDRESS[chain ? chain.id : SUPPORT_CHAIN_IDS.MUMBAI].substring(2)}`,
    abi: ERC20_ABI,
    functionName: "mint"
  });
  const { writeAsync: mintB } = useContractWrite({
    address: `0x${BTOKEN_ADDRESS[chain ? chain.id : SUPPORT_CHAIN_IDS.MUMBAI].substring(2)}`,
    abi: ERC20_ABI,
    functionName: "mint"
  });
